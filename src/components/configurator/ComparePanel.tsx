    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      <div className="grid gap-px bg-border md:grid-cols-2">
        <SlotCard
          ...
          onRequestQuote={() => onRequestQuote("A")}
        />
        <SlotCard
          id="B"
          slot={B}
          ...
          onRequestQuote={() => onRequestQuote("B")}
        />
      </div>
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 text-surface-foreground">
          <p className="text-[10px] font-semibold uppercase tracking-wider opacity-80">
            {t(`categories.${slot.model.category}`)}
          </p>
          <p className="font-semibold leading-tight">{slot.model.name}</p>
          <p className="text-xs opacity-70">
            {pick(slot.model.tagline)} · {slot.model.capacity}
          </p>
        </div>
      </button>
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-1.5 flex items-center justify-between text-xs">
          <span className="text-muted-foreground">
            {slot.chosen.length}{" "}
            {slot.chosen.length === 1 ? t("cfg.optionSelected") : t("cfg.optionsSelected")}
          </span>
          <span className="font-medium tabular-nums text-muted-foreground">
            +
            {formatEUR(
              slot.chosen.reduce((s, o) => s + o.price, 0),
              lang,
            )}
          </span>
        </div>
...
function DiffPill({
  ...
  return (
    <span
      className={cn(
        base,
        present ? onClasses : offClasses,
        highlight && present && "ring-2 ring-offset-1 ring-offset-card",
