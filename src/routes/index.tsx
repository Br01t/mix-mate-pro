...
import { ArrowRight, Gauge, Target, ShieldCheck, Cpu, Sparkles, Factory, FlaskConical, Beaker } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Mixer industriali twin-rotor brevettati per precisione, sicurezza e continuità operativa. Configura la tua macchina online." },
      { property: "og:title", content: "MixCore — Tecnologia brevettata di miscelazione industriale" },
      { property: "og:description", content: "Mixer industriali twin-rotor brevettati per precisione, sicurezza e continuità operativa." },
    ],
  }),
  component: HomePage,
});
    ...
    <>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="mb-12 max-w-2xl">
          <p className="text-sm font-semibold uppercase tracking-wider text-primary">{t("home.whyKicker")}</p>
          <h2 className="mt-2 text-3xl font-semibold tracking-tight md:text-4xl">{t("home.whyTitle")}</h2>
        ...
        <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="max-w-xl">
              <p className="text-sm font-semibold uppercase tracking-wider text-primary">{t("home.howKicker")}</p>
              <h2 className="mt-2 text-3xl font-semibold tracking-tight md:text-4xl">{t("home.howTitle")}</h2>
            </div>
            <Button asChild variant="outline">
              <Link to="/technology">{t("home.fullBreakdown")} <ArrowRight className="ml-2 h-4 w-4" /></Link>
            </Button>
          </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-4 py-20 sm:px-6 md:grid-cols-2 lg:px-8">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wider text-primary">{t("home.aboutKicker")}</p>
          <h2 className="mt-2 text-3xl font-semibold tracking-tight md:text-4xl">{t("home.aboutTitle")}</h2>
          <p className="mt-4 text-muted-foreground">{t("home.aboutBody")}</p>
          <div className="mt-6 flex gap-3">
            <Button asChild><Link to="/configurator">{t("common.startConfiguring")}</Link></Button>
            <Button asChild variant="outline"><Link to="/technology">{t("common.readSpecs")}</Link></Button>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4">
          {industries.map((i) => (
            <div key={i.label} className="flex flex-col items-start gap-3 rounded-xl border border-border bg-card p-6">
              ...
              <p className="text-sm text-muted-foreground">{t("home.industryDesc")} {i.label.toLowerCase()}.</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="border-y border-border bg-muted/40">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-6 px-4 py-10 sm:px-6 lg:px-8">
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("home.trustedBy")}</p>
