export type Lang = "it" | "en";

export const dict = {
  common: {
    requestQuote: { it: "Richiedi preventivo", en: "Request Quote" },
    openConfigurator: { it: "Apri configuratore", en: "Open Configurator" },
    configureMixer: { it: "Configura il tuo mixer", en: "Configure your mixer" },
    seeTechnology: { it: "Scopri la tecnologia", en: "See the technology" },
    readSpecs: { it: "Vedi scheda tecnica", en: "Read the spec sheet" },
    startConfiguring: { it: "Inizia a configurare", en: "Start configuring" },
    learnMore: { it: "Approfondisci", en: "Learn more" },
    close: { it: "Chiudi", en: "Close" },
    cancel: { it: "Annulla", en: "Cancel" },
    submit: { it: "Invia richiesta", en: "Submit request" },
    optional: { it: "facoltativo", en: "optional" },
  },
  nav: {
    home: { it: "Home", en: "Home" },
    technology: { it: "Tecnologia", en: "Technology" },
    configurator: { it: "Configuratore", en: "Configurator" },
    contact: { it: "Contatti", en: "Contact" },
  },
  footer: {
    tagline: {
      it: "Tecnologia brevettata di miscelazione industriale per precisione, sicurezza e continuità operativa.",
      en: "Patented industrial mixing technology engineered for precision, safety and uptime.",
    },
    product: { it: "Prodotto", en: "Product" },
    company: { it: "Azienda", en: "Company" },
    contact: { it: "Contatti", en: "Contact" },
    about: { it: "Chi siamo", en: "About" },
    careers: { it: "Lavora con noi", en: "Careers" },
    press: { it: "Stampa", en: "Press" },
    certifications: { it: "Certificazioni", en: "Certifications" },
    rights: { it: "Tutti i diritti riservati.", en: "All rights reserved." },
    legal: { it: "Privacy · Termini · Cookie", en: "Privacy · Terms · Cookies" },
  },
  home: {
    badge: {
      it: "Brevetto EP3 482 109 — Miscelazione twin-rotor",
      en: "Patent EP3 482 109 — Twin-rotor mixing",
    },
    h1a: { it: "Miscelazione industriale,", en: "Industrial mixing," },
    h1b: { it: "ingegnerizzata al micron.", en: "engineered to the micron." },
    intro: {
      it: "Il nostro processo brevettato twin-rotor offre maggiore portata, precisione superiore e ripetibilità senza pari — per le linee produttive più esigenti al mondo.",
      en: "Our patented twin-rotor process delivers higher throughput, sharper precision and unmatched repeatability — for the world's most demanding production lines.",
    },
    statDeployed: { it: "Unità installate", en: "Units deployed" },
    statCountries: { it: "Paesi", en: "Countries" },
    statEng: { it: "Ingegneria", en: "Engineering" },
    statEngVal: { it: "18 anni", en: "18 yr" },
    whyKicker: { it: "Perché MixCore", en: "Why MixCore" },
    whyTitle: {
      it: "Un nuovo standard per la miscelazione industriale.",
      en: "A new standard for industrial mixing.",
    },
    whyIntro: {
      it: "Costruito attorno a un'unica innovazione brevettata, ogni unità MixCore alza portata, precisione e continuità in un solo pacchetto ingegnerizzato.",
      en: "Built around a single patented innovation, every MixCore unit raises throughput, precision and uptime in one engineered package.",
    },
    benefit1Title: { it: "+38% di portata", en: "+38% throughput" },
    benefit1Desc: {
      it: "La geometria twin-rotor brevettata garantisce tassi di miscelazione superiori a minor consumo energetico.",
      en: "Patented twin-rotor geometry delivers higher mixing rates at lower energy.",
    },
    benefit2Title: { it: "Precisione sub-micron", en: "Sub-micron precision" },
    benefit2Desc: {
      it: "Omogeneizzazione fino a 0,8 µm per riproducibilità di livello farmaceutico.",
      en: "Homogenization down to 0.8 µm for pharma-grade reproducibility.",
    },
    benefit3Title: { it: "Sicurezza certificata", en: "Certified safety" },
    benefit3Desc: {
      it: "Conformità ATEX, CE, FDA e 3-A sanitary su tutta la gamma.",
      en: "ATEX, CE, FDA & 3-A sanitary compliance across the product line.",
    },
    benefit4Title: { it: "Controllo intelligente", en: "Smart control" },
    benefit4Desc: {
      it: "PLC IoT-ready con gestione ricette e manutenzione predittiva.",
      en: "IoT-ready PLC with recipe management and predictive maintenance.",
    },
    howKicker: { it: "Come funziona", en: "How it works" },
    howTitle: { it: "Tre fasi ingegnerizzate.", en: "Three engineered stages." },
    fullBreakdown: { it: "Approfondimento tecnico completo", en: "Full technical breakdown" },
    step1Title: { it: "Iniezione in controcorrente", en: "Counter-flow injection" },
    step1Desc: {
      it: "Le sostanze entrano da bocchette opposte, pre-condizionate per taglio e temperatura.",
      en: "Substances enter through opposing inlets, pre-conditioned for shear and temperature.",
    },
    step2Title: { it: "Miscelazione twin-rotor", en: "Twin-rotor blending" },
    step2Desc: {
      it: "La cella twin-rotor brevettata genera micro-vortici, eliminando le zone morte.",
      en: "Patented twin-rotor cell creates micro-vortices, eliminating dead zones.",
    },
    step3Title: { it: "Verifica inline", en: "Inline verification" },
    step3Desc: {
      it: "Sensori ottici validano l'omogeneità in tempo reale prima dello scarico.",
      en: "Optical sensors validate homogeneity in real time before discharge.",
    },
    aboutKicker: { it: "Chi è MixCore", en: "About MixCore" },
    aboutTitle: {
      it: "18 anni di ingegneria della miscelazione, un brevetto che ha cambiato lo standard.",
      en: "18 years of mixing engineering, one patent that changed the standard.",
    },
    aboutBody: {
      it: "Fondata nel 2007 a Milano, MixCore progetta e produce sistemi di miscelazione industriale per settori regolamentati. La nostra cella twin-rotor brevettata (EP3 482 109) è oggi installata su linee farmaceutiche, alimentari, chimiche e petrolchimiche in 42 paesi.",
      en: "Founded in 2007 in Milan, MixCore designs and manufactures industrial mixing systems for regulated industries. Our patented twin-rotor cell (EP3 482 109) is now deployed across pharma, food, chemical and petrochemical production lines in 42 countries.",
    },
    trustedBy: {
      it: "Scelti da produttori in 42 paesi",
      en: "Trusted by manufacturers in 42 countries",
    },
    industryDesc: {
      it: "Ingegnerizzato per le esigenze normative e di processo della produzione",
      en: "Engineered for the regulatory and process demands of",
    },
    ctaTitle: {
      it: "Configura il tuo mixer. Preventivo in 24 ore.",
      en: "Build your mixer. Get a quote in 24 hours.",
    },
    ctaBody: {
      it: "Configura modello e opzioni in pochi minuti. Il nostro team di ingegneria revisiona ogni richiesta e risponde con una proposta su misura.",
      en: "Configure your model and options in minutes. Our engineering team reviews every request and replies with a tailored proposal.",
    },
  },
  tech: {
    kicker: { it: "Tecnologia brevettata", en: "Patented technology" },
    h1: {
      it: "La cella di miscelazione twin-rotor. Un brevetto che riprogetta l'omogeneizzazione.",
      en: "The twin-rotor mixing cell. A patent that re-engineers homogenization.",
    },
    intro: {
      it: "Il nostro brevetto EP3 482 109 sostituisce la turbolenza monoalbero con un'architettura twin-rotor sincronizzata — più portata, precisione più fine e zero zone morte su tutto il campo di viscosità.",
      en: "Our EP3 482 109 patent replaces single-shaft turbulence with a synchronized twin-rotor architecture — delivering higher throughput, finer precision and zero dead zones across the full viscosity range.",
    },
    howKicker: { it: "Come avviene il processo", en: "How the process works" },
    howTitle: { it: "Quattro fasi, un loop chiuso.", en: "Four stages, one closed loop." },
    p1Title: { it: "Iniezione in controcorrente", en: "Counter-flow injection" },
    p1Desc: {
      it: "Due sostanze entrano nella camera da bocchette opposte, ognuna pre-condizionata per taglio, temperatura e portata.",
      en: "Two substances enter the mixing chamber through opposing inlets, each pre-conditioned for shear, temperature and flow rate.",
    },
    p2Title: { it: "Vortice twin-rotor", en: "Twin-rotor vortex" },
    p2Desc: {
      it: "La geometria twin-rotor brevettata genera micro-vortici sincronizzati, eliminando zone morte e sovra-taglio.",
      en: "Our patented twin-rotor geometry generates synchronized micro-vortices, eliminating dead zones and overshear.",
    },
    p3Title: { it: "Verifica inline", en: "Inline verification" },
    p3Desc: {
      it: "Sensori ottici e ultrasonici validano la distribuzione delle particelle in tempo reale prima dello scarico.",
      en: "Optical and ultrasonic sensors validate particle distribution in real time before discharge.",
    },
    p4Title: { it: "Controllo in anello chiuso", en: "Closed-loop control" },
    p4Desc: {
      it: "Il PLC regola al volo la velocità del rotore e i tempi di permanenza per restare nelle tolleranze di ricetta.",
      en: "The PLC adjusts rotor speed and dwell time on the fly to hold the recipe within target tolerance.",
    },
    specsKicker: { it: "Specifiche tecniche", en: "Technical specifications" },
    specsTitle: { it: "Range ingegnerizzato.", en: "Engineered envelope." },
    specsIntro: {
      it: "Le unità MixCore coprono l'intero spettro dal pilota R&D alla produzione di stabilimento, con materiali e certificazioni scelti per applicazione.",
      en: "MixCore units cover the full range from R&D pilot to plant-scale production, with materials and certifications selected per application.",
    },
    configureSpecs: { it: "Configura le tue specifiche", en: "Configure your specs" },
    casesKicker: { it: "Casi d'uso", en: "Use cases" },
    casesTitle: { it: "Settori per cui progettiamo.", en: "Industries we engineer for." },
    spec: {
      throughput: {
        k: { it: "Portata", en: "Throughput" },
        v: { it: "50 — 5.000 L per batch", en: "50 — 5,000 L per batch" },
      },
      precision: {
        k: { it: "Precisione di miscelazione", en: "Mixing precision" },
        v: { it: "Omogeneizzazione fino a 0,8 µm", en: "Down to 0.8 µm homogenization" },
      },
      viscosity: {
        k: { it: "Range di viscosità", en: "Viscosity range" },
        v: { it: "1 cP — 250.000 cP", en: "1 cP — 250,000 cP" },
      },
      power: {
        k: { it: "Potenza motore", en: "Drive power" },
        v: {
          it: "1,5 — 75 kW (a frequenza controllata)",
          en: "1.5 — 75 kW (frequency controlled)",
        },
      },
      materials: {
        k: { it: "Materiali a contatto", en: "Contact materials" },
        v: { it: "AISI 304 / 316L / Hastelloy C-22", en: "AISI 304 / 316L / Hastelloy C-22" },
      },
      temp: {
        k: { it: "Temperatura d'esercizio", en: "Operating temperature" },
        v: { it: "-10 °C a +180 °C", en: "-10 °C to +180 °C" },
      },
      pressure: {
        k: { it: "Pressione di lavoro", en: "Working pressure" },
        v: { it: "Fino a 6 bar (camicia)", en: "Up to 6 bar (jacketed)" },
      },
      certs: {
        k: { it: "Certificazioni", en: "Certifications" },
        v: { it: "CE, ATEX, FDA, 3-A sanitary, IECEx", en: "CE, ATEX, FDA, 3-A sanitary, IECEx" },
      },
      control: {
        k: { it: "Sistema di controllo", en: "Control system" },
        v: {
          it: "Siemens S7-1500 / Rockwell ControlLogix",
          en: "Siemens S7-1500 / Rockwell ControlLogix",
        },
      },
      connect: {
        k: { it: "Connettività", en: "Connectivity" },
        v: { it: "OPC-UA, Modbus TCP, REST API, MQTT", en: "OPC-UA, Modbus TCP, REST API, MQTT" },
      },
    },
    industries: {
      pharma: {
        n: { it: "Farmaceutico", en: "Pharmaceutical" },
        d: {
          it: "Formulazione di API, adiuvanti vaccinali, build sanitari sterili con piena tracciabilità.",
          en: "API formulation, vaccine adjuvants, sterile sanitary builds with full traceability.",
        },
      },
      food: {
        n: { it: "Food & Beverage", en: "Food & Beverage" },
        d: {
          it: "Emulsioni, salse, latticini e blend vegetali con pulizia CIP/SIP.",
          en: "Emulsions, sauces, dairy and plant-based blends with CIP/SIP cleaning.",
        },
      },
      chem: {
        n: { it: "Chimico", en: "Chemicals" },
        d: {
          it: "Miscelazione reattiva, dispersione polimeri, omogeneizzazione catalizzatori in scala.",
          en: "Reactive mixing, polymer dispersion, catalyst homogenization at scale.",
        },
      },
      cosm: {
        n: { it: "Cosmetica", en: "Cosmetics" },
        d: {
          it: "Creme, gel e lozioni ad alta viscosità con aerazione controllata.",
          en: "High-viscosity creams, gels and lotions with controlled aeration.",
        },
      },
      petro: {
        n: { it: "Petrolchimico", en: "Petrochemical" },
        d: {
          it: "Miscelazione di solventi e additivi certificata ATEX per ambienti pericolosi.",
          en: "ATEX-rated solvent and additive blending for hazardous environments.",
        },
      },
      constr: {
        n: { it: "Edilizia", en: "Construction" },
        d: {
          it: "Slurry abrasive, malte e sistemi resinosi con drive train rinforzato.",
          en: "Abrasive slurries, mortars and resin systems with reinforced drive train.",
        },
      },
    },
  },
  cfg: {
    kicker: { it: "Configuratore macchina", en: "Machine configurator" },
    title: { it: "Costruisci il tuo mixer MixCore.", en: "Build your MixCore mixer." },
    intro: {
      it: "Scegli un modello, attiva le opzioni che ti servono e guarda il totale aggiornarsi in tempo reale. Quando sei pronto, richiedi un preventivo ufficiale al nostro team di ingegneria.",
      en: "Pick a model, toggle the options you need, and see your total update live. When you're ready, request an official quote from our engineering team.",
    },
    step1: { it: "Scegli il modello", en: "Choose your model" },
    step2: { it: "Aggiungi optional", en: "Add optional features" },
    filterAll: { it: "Tutti", en: "All" },
    yourConfig: { it: "La tua configurazione", en: "Your configuration" },
    basePrice: { it: "Prezzo base", en: "Base price" },
    estimatedTotal: { it: "Totale stimato", en: "Estimated total" },
    priceNote: {
      it: "Prezzo indicativo, IVA esclusa. Il preventivo finale include trasporto e messa in servizio.",
      en: "Indicative price, VAT excluded. Final quote includes shipping & commissioning.",
    },
    requestOfficial: { it: "Richiedi preventivo ufficiale", en: "Request Official Quote" },
    capacity: { it: "Capacità", en: "Capacity" },
    industry: { it: "Settore", en: "Industry" },
    optionsSelected: { it: "opzioni selezionate", en: "options selected" },
    optionSelected: { it: "opzione selezionata", en: "option selected" },
    noOptions: { it: "Nessun optional selezionato", en: "No optionals selected" },
    selectAll: { it: "Seleziona tutti", en: "Select all" },
    clearAll: { it: "Rimuovi tutti", en: "Clear all" },
    reset: { it: "Ripristina", en: "Reset" },
    selected: { it: "Selezionato", en: "Selected" },
    select: { it: "Seleziona", en: "Select" },
    saving: { it: "Risparmio", en: "Savings" },
    compareMode: { it: "Modalità confronto", en: "Compare mode" },
    compareHint: {
      it: "Confronta due configurazioni affiancate",
      en: "Compare two configurations side by side",
    },
    slot: { it: "Configurazione", en: "Configuration" },
    activeSlot: { it: "Configurazione attiva", en: "Active configuration" },
    editing: { it: "In modifica", en: "Editing" },
    duplicateToB: { it: "Copia A → B", en: "Copy A → B" },
    swapAB: { it: "Scambia A ⇄ B", en: "Swap A ⇄ B" },
    diffOnlyIn: { it: "solo in", en: "only in" },
    inBoth: { it: "in entrambe", en: "in both" },
    priceDelta: { it: "Differenza", en: "Difference" },
    sameModel: { it: "Stesso modello", en: "Same model" },
    diffModel: { it: "Modelli diversi", en: "Different models" },
    cheaper: { it: "più economica", en: "cheaper" },
    moreExpensive: { it: "più costosa", en: "more expensive" },
    requestThis: { it: "Richiedi questa", en: "Request this one" },
  },
  quote: {
    title: { it: "Richiedi preventivo ufficiale", en: "Request official quote" },
    desc: {
      it: "Inserisci i tuoi dati — il nostro team risponderà con una proposta su misura entro un giorno lavorativo.",
      en: "Submit your details — our team will reply with a tailored proposal within one business day.",
    },
    fullName: { it: "Nome e cognome", en: "Full name" },
    company: { it: "Azienda", en: "Company" },
    email: { it: "Email aziendale", en: "Work email" },
    phone: { it: "Telefono", en: "Phone" },
    notes: { it: "Note", en: "Notes" },
    notesPh: {
      it: "Applicazione, portata target, tempistiche…",
      en: "Application, target throughput, timeline…",
    },
    received: { it: "Richiesta ricevuta", en: "Quote request received" },
    receivedBody: {
      it: "Il nostro team commerciale-tecnico ti risponderà entro un giorno lavorativo.",
      en: "Our sales engineering team will reply within one business day.",
    },
    reference: { it: "Riferimento", en: "Reference" },
    model: { it: "Modello", en: "Model" },
    options: { it: "Optional", en: "Options" },
    total: { it: "Totale", en: "Total" },
    configAnother: { it: "Configura un altro", en: "Configure another" },
    errors: {
      name: { it: "Inserisci nome e cognome", en: "Please enter your full name" },
      company: { it: "Il nome azienda è obbligatorio", en: "Company name is required" },
      email: { it: "Inserisci una email valida", en: "Enter a valid email" },
      phone: { it: "Inserisci un telefono valido", en: "Enter a valid phone number" },
    },
  },
  categories: {
    lab: { it: "Laboratorio", en: "Lab" },
    pilot: { it: "Pilota", en: "Pilot" },
    production: { it: "Produzione", en: "Production" },
    heavy: { it: "Heavy duty", en: "Heavy duty" },
    flagship: { it: "Flagship", en: "Flagship" },
  },
  contact: {
    kicker: { it: "Parliamo del tuo processo", en: "Let's talk about your process" },
    title: {
      it: "Prenota una consulenza con i nostri ingegneri.",
      en: "Book a consultation with our engineers.",
    },
    intro: {
      it: "Un confronto tecnico di 30 minuti, online o nel nostro stabilimento di Milano, per valutare insieme l'applicazione e la macchina più adatta.",
      en: "A 30-minute technical conversation, online or at our Milan facility, to scope your application and the right machine for it.",
    },
    infoTitle: { it: "Informazioni di contatto", en: "Contact information" },
    infoBody: {
      it: "Il nostro team risponde in italiano e inglese, dal lunedì al venerdì, 9:00 – 18:00 CET.",
      en: "Our team replies in Italian and English, Monday to Friday, 9:00 am – 6:00 pm CET.",
    },
    hq: { it: "Sede operativa", en: "Headquarters" },
    hqAddr: {
      it: "Via dell'Industria 14, 20010 Milano, Italia",
      en: "Via dell'Industria 14, 20010 Milan, Italy",
    },
    phoneLabel: { it: "Centralino", en: "Switchboard" },
    emailLabel: { it: "Email commerciale", en: "Sales email" },
    hoursLabel: { it: "Orari", en: "Hours" },
    hoursValue: { it: "Lun–Ven · 9:00 — 18:00 CET", en: "Mon–Fri · 9:00 am — 6:00 pm CET" },
    formTitle: { it: "Richiedi un appuntamento", en: "Request an appointment" },
    formIntro: {
      it: "Ti ricontattiamo entro un giorno lavorativo per confermare data e orario.",
      en: "We'll get back to you within one business day to confirm date and time.",
    },
    fName: { it: "Nome e cognome", en: "Full name" },
    fCompany: { it: "Azienda", en: "Company" },
    fEmail: { it: "Email aziendale", en: "Work email" },
    fPhone: { it: "Telefono", en: "Phone" },
    fRole: { it: "Ruolo (facoltativo)", en: "Role (optional)" },
    fIndustry: { it: "Settore", en: "Industry" },
    fIndustryPh: { it: "Es. Farmaceutico, Food, Chimico…", en: "e.g. Pharma, Food, Chemicals…" },
    fType: { it: "Tipo di appuntamento", en: "Appointment type" },
    typeOnline: { it: "Online (video call)", en: "Online (video call)" },
    typeOnlineDesc: {
      it: "Su Google Meet o Microsoft Teams, link via email.",
      en: "Via Google Meet or Microsoft Teams, link sent by email.",
    },
    typeOnsite: { it: "In sede a Milano", en: "On-site in Milan" },
    typeOnsiteDesc: {
      it: "Visita guidata allo showroom e alla linea pilota.",
      en: "Guided tour of the showroom and pilot line.",
    },
    typeVisit: { it: "Visita nella tua sede", en: "Visit at your site" },
    typeVisitDesc: {
      it: "Un nostro ingegnere viene da te per un sopralluogo.",
      en: "One of our engineers comes to you for an on-site assessment.",
    },
    fDate: { it: "Data preferita", en: "Preferred date" },
    fTime: { it: "Fascia oraria", en: "Time slot" },
    timeMorning: { it: "Mattina (9:00 — 12:00)", en: "Morning (9:00 — 12:00)" },
    timeAfternoon: { it: "Pomeriggio (14:00 — 18:00)", en: "Afternoon (2:00 — 6:00 pm)" },
    timeFlexible: { it: "Flessibile", en: "Flexible" },
    fMessage: { it: "Descrivi la tua esigenza", en: "Describe your need" },
    fMessagePh: {
      it: "Applicazione, viscosità, portata desiderata, vincoli normativi…",
      en: "Application, viscosity, target throughput, regulatory constraints…",
    },
    fConsent: {
      it: "Accetto l'informativa privacy e il trattamento dei miei dati per essere ricontattato.",
      en: "I accept the privacy notice and the processing of my data to be contacted.",
    },
    submit: { it: "Prenota appuntamento", en: "Book appointment" },
    submitting: { it: "Invio in corso…", en: "Sending…" },
    successTitle: { it: "Richiesta ricevuta", en: "Request received" },
    successBody: {
      it: "Ti ricontattiamo entro un giorno lavorativo per confermare data e orario.",
      en: "We'll contact you within one business day to confirm date and time.",
    },
    reference: { it: "Riferimento", en: "Reference" },
    newRequest: { it: "Invia un'altra richiesta", en: "Send another request" },
    err: {
      name: { it: "Inserisci nome e cognome", en: "Please enter your full name" },
      company: { it: "Il nome azienda è obbligatorio", en: "Company name is required" },
      email: { it: "Inserisci una email valida", en: "Enter a valid email" },
      phone: { it: "Inserisci un telefono valido", en: "Enter a valid phone number" },
      date: { it: "Seleziona una data", en: "Pick a date" },
      message: { it: "Aggiungi qualche dettaglio", en: "Add a few details" },
      consent: { it: "È necessario accettare l'informativa", en: "You must accept the notice" },
    },
  },
} as const;

export type Dict = typeof dict;
